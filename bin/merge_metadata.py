...
def parse():
    ...
    parser.add_argument("metadata_file", help="contains clone ID and SRR ID for WGS data")
    parser.add_argument("sample_ID_file", help="contains DNA > RNA sample ID mappings")
    parser.add_argument("data_dir", help="Directory containing downloaded RNA-seq data")
...
def merge_meta(metadata_file, sample_ID_file, data_dir, outf):
    ...
    id_map_dat = id_map_dat.rename(columns={"sample": "rna_sample_id", "sample_title": "sample_name"})
    ...
    merged_meta = merged_meta[["rna_sample_id"] + [col for col in merged_meta.columns if col != "rna_sample_id"]]
